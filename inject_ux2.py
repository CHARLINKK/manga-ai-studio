...
shutil.copy('app.py.bak4', 'app.py')
...
# 5. Add Tooltips to complex elements safely (match until the newline)
...
code = re.sub(r'self\.sw_rag = ctk\.CTkSwitch[^\n]+', inject_rag_tooltip, code)
...
code = re.sub(r'self\.switch_gpu = ctk\.CTkSwitch[^\n]+', inject_gpu_tooltip, code)
...
code = re.sub(r'self\.tb_dict_global = ctk\.CTkTextbox[^\n]+', inject_dict_tooltip, code)
