...
class DraggableBlock(ctk.CTkFrame):
    def __init__(self, master, text_content, parent_app, original_text="", **kwargs):
        ...
        self.handle = ctk.CTkLabel(self, text="☰", cursor="fleur", text_color=("#888", "#777"), font=ctk.CTkFont(size=20, weight="bold"), width=30)
        ...
        def set_dirty(event):
            if hasattr(self.parent_app, 'studio_is_dirty'):
                self.parent_app.studio_is_dirty = True
        self.textbox.bind("<KeyRelease>", set_dirty)
...
def ask_string(title, prompt, parent=None):
    ...
    ctk.CTkLabel(dialog, text=prompt, font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(25, 10), padx=20)
    entry = ctk.CTkEntry(dialog, width=350, font=ctk.CTkFont(size=13))
    ...
    ctk.CTkButton(btn_frame, text="Confirmar", width=120, command=on_ok).pack(side="left", padx=10)
    ctk.CTkButton(btn_frame, text="Cancelar", width=120, fg_color="#c0392b", hover_color="#e74c3c", command=on_cancel).pack(side="left", padx=10)
...
class MangaApp(ctk.CTk):
    def __init__(self):
        ...
        # Carregar Configurações (Temas)
        import os
        import json
        from pathlib import Path
        self.settings_file = Path(os.environ.get("USERPROFILE", "")) / "Documents" / "Manga AI Studio" / "settings.json"
        self.app_settings = {"theme": "Dark", "accent": "blue"}
        ...
        # Aplicar Temas IMEDIATAMENTE antes da UI nascer
        ctk.set_appearance_mode(self.app_settings["theme"])
        try:
            ctk.set_default_color_theme(self.app_settings["accent"])
        except Exception:
            pass
        ...
        # Barra Lateral (Workspace Explorer) - Agora mais larga por padrão (320px)
        self.sidebar_frame = ctk.CTkFrame(self.root_frame, width=320, corner_radius=0)
        ...
        self.btn_set_workspace = ctk.CTkButton(self.sidebar_frame, text=" 🏠 Meu Computador", fg_color=("#ddd", "#444"), hover_color=("#ccc", "#555"), text_color=("#000", "#fff"), command=self.go_to_virtual_root)
        ...
        self.btn_refresh = ctk.CTkButton(header_actions, text="↻", width=36, font=ctk.CTkFont(size=22, weight="bold"), fg_color=("#ddd", "#444"), hover_color=("#ccc", "#555"), text_color=("#000", "#fff"), command=self.refresh_workspace)
        ...
        self.explorer_header_frame = ctk.CTkFrame(self.sidebar_frame, fg_color="transparent")
        self.explorer_header_frame.pack(fill="x", padx=10, pady=(5, 0))
        
        self.workspace_list = ctk.CTkScrollableFrame(self.sidebar_frame, fg_color="transparent")
        self.workspace_list.pack(fill="both", expand=True, padx=5, pady=5)
        
        # Acelerar velocidade do scroll (Monkey patch agressivo)
        original_mouse_wheel = self.workspace_list._mouse_wheel_all
        def accelerated_mouse_wheel(event):
            try:
                event.delta = int(event.delta * 20) # 20x mais rápido no Windows
            except Exception:
                pass
            original_mouse_wheel(event)
        self.workspace_list._mouse_wheel_all = accelerated_mouse_wheel
        ...
        # Barra de Abas Customizada (Zero Lag)
        self.tab_bar = ctk.CTkFrame(self.main_content_frame, fg_color="transparent", height=40)
        self.tab_bar.pack(fill="x", padx=20, pady=(10, 0))
        
        self.tab_container = ctk.CTkFrame(self.main_content_frame, fg_color="transparent")
        self.tab_container.pack(fill="both", expand=True, padx=20, pady=(5, 10))
        
        self.current_tab = "Processamento"
        self.previous_tab = "Processamento"
        
        # Botão de Toggle do Explorador (Esconder/Mostrar)
        def toggle_explorer():
            if self.sidebar_frame.winfo_ismapped():
                self.sidebar_frame.pack_forget()
                self.btn_toggle_explorer.configure(text="▶")
            else:
                self.sidebar_frame.pack(side="left", fill="y", before=self.main_content_frame)
                self.btn_toggle_explorer.configure(text="◀")
                
        self.btn_toggle_explorer = ctk.CTkButton(self.tab_bar, text="◀", width=30, font=ctk.CTkFont(size=14, weight="bold"),
                                                fg_color=("#ccc", "#444"), hover_color=("#bbb", "#555"), text_color=("#000", "#fff"), command=toggle_explorer)
        self.btn_toggle_explorer.pack(side="left", padx=(0, 10))
        
        self.tab_buttons = {}
        
        # Frame centralizado absolutamente para abrigar as abas
        self.tab_center_frame = ctk.CTkFrame(self.tab_bar, fg_color="transparent")
        self.tab_center_frame.place(relx=0.5, rely=0.5, anchor="center")
        
        tab_names = ["Processamento", "Estúdio de Tradução", "Leitor", "Central de Módulos", "Configurações"]
        for name in tab_names:
            btn = ctk.CTkButton(self.tab_center_frame, text=name, font=ctk.CTkFont(size=14, weight="bold"), 
                                fg_color=("#ddd", "#333"), hover_color=("#ccc", "#444"), text_color=("#000", "#aaa"), corner_radius=5,
                                command=lambda n=name: self.set_tab(n))
            btn.pack(side="left", padx=2)
            self.tab_buttons[name] = btn
            
        self.tab_frames = {}
        self.tab_process = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.tab_frames["Processamento"] = self.tab_process
        
        self.tab_studio = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.tab_frames["Estúdio de Tradução"] = self.tab_studio
        
        self.tab_reader = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.tab_frames["Leitor"] = self.tab_reader
        
        self.tab_modules = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.tab_frames["Central de Módulos"] = self.tab_modules
        
        self.tab_settings = ctk.CTkFrame(self.tab_container, fg_color="transparent")
        self.tab_frames["Configurações"] = self.tab_settings
        ...
        try:
            ...
            def on_drop(files):
                if not files: return
                path = files[0].decode('mbcs', errors='ignore')
                if os.path.isdir(path):
                    if self.current_tab == "Processamento":
    ...
    def on_closing(self):
        ...
        if getattr(self, 'studio_is_dirty', False):
            if not messagebox.askyesno("Aviso", "Existem alterações não salvas no Estúdio. Deseja sair mesmo assim e perder o progresso?"):
                return
        self.destroy()

    def set_tab(self, target_name):
        if hasattr(self, 'previous_tab') and self.previous_tab == "Estúdio de Tradução" and target_name != "Estúdio de Tradução":
            if getattr(self, 'studio_is_dirty', False):
                from tkinter import messagebox
                if not messagebox.askyesno("Aviso", "Atenção: Progresso não exportado será perdido caso não retorne para salvar.\nDeseja mudar de aba mesmo assim?"):
                    return # Cancela a troca
                    
        self.current_tab = target_name
        self.previous_tab = target_name
        
        # Atualiza botões
        for name, btn in self.tab_buttons.items():
            if name == target_name:
                btn.configure(fg_color="#3498db", text_color=("#000", "#fff")) # Azul ativo
            else:
                btn.configure(fg_color=("#ffffff", "#333"), text_color=("#666", "#aaa")) # Cinza inativo
                
        # Traz o frame pro topo (magia do zero-lag)
        if target_name in self.tab_frames:
            self.tab_frames[target_name].lift()

    def set_workspace(self):
        path = filedialog.askdirectory(title="Selecione a Pasta Mãe dos Projetos")
        if path:
            self.workspace_path = Path(path)
            self.current_explorer_path = self.workspace_path
    ...
    def go_to_virtual_root(self):
        self.current_explorer_path = "MEU_COMPUTADOR"
        self.refresh_workspace()

    def explorer_go_back(self):
        if self.current_explorer_path == "MEU_COMPUTADOR":
            return
            
        if self.current_explorer_path and self.current_explorer_path.parent:
            if str(self.current_explorer_path) == str(self.current_explorer_path.parent):
                self.current_explorer_path = "MEU_COMPUTADOR"
            else:
    ...
    def _render_virtual_root(self):
        lbl_breadcrumb = ctk.CTkLabel(self.explorer_header_frame, text="MEU COMPUTADOR", font=ctk.CTkFont(size=16, weight="bold"), text_color=("#000", "#ffffff"))
        lbl_breadcrumb.pack(side="left", padx=(5, 0))
        
        btn_set_ws = ctk.CTkButton(self.explorer_header_frame, text="Definir Pasta Base Fixa...", fg_color=("#ddd", "#444"), hover_color=("#ccc", "#555"), text_color=("#000", "#fff"), command=self.set_workspace)
        btn_set_ws.pack(fill="x", pady=(0, 10), padx=0)
        
        import string
        import os
        drives = [f"{d}:\\" for d in string.ascii_uppercase if os.path.exists(f"{d}:\\")]
        for d in drives:
            self.create_workspace_item(Path(d), custom_name=f"Disco Local ({d[:2]})", custom_icon="💾", force_container=True)
        ...
        if desktop.exists(): self.create_workspace_item(desktop, custom_name="Área de Trabalho", custom_icon="🖥️", force_container=True)
        if downloads.exists(): self.create_workspace_item(downloads, custom_name="Downloads", custom_icon="⬇️", force_container=True)
        if docs.exists(): self.create_workspace_item(docs, custom_name="Documentos", custom_icon="📄", force_container=True)

    def refresh_workspace(self):
        if not hasattr(self, 'thumbnail_queue'):
            self.thumbnail_queue = []
        self.thumbnail_queue.clear() # Limpa a fila ao mudar de diretório
        
        for widget in self.workspace_list.winfo_children():
            widget.destroy()
            
        if hasattr(self, 'explorer_header_frame'):
            for widget in self.explorer_header_frame.winfo_children():
                widget.destroy()
            
        if getattr(self, 'current_explorer_path', None) is None:
            self.current_explorer_path = self.workspace_path
            
        if self.current_explorer_path == "MEU_COMPUTADOR":
            self._render_virtual_root()
            return
            
        if not self.current_explorer_path or not self.current_explorer_path.exists():
            return
            
        # Trilha de Navegação (Breadcrumb Inteligente)
        if self.current_explorer_path:
            full_path = str(self.current_explorer_path)
            
            def shorten_path(p_str, max_len=28):
                if len(p_str) <= max_len: return p_str
                parts = p_str.replace('\\', '/').split('/')
                if len(parts) <= 2: return "..." + p_str[-(max_len-3):]
                first, last = parts[0], parts[-1]
                if len(last) > max_len - 6: return f".../{last[-(max_len-5):]}"
                mid_parts = []
                cur_len = len(first) + len(last) + 5
                for p in reversed(parts[1:-1]):
                    if cur_len + len(p) + 1 <= max_len:
                        mid_parts.insert(0, p)
                        cur_len += len(p) + 1
                    else: break
                mid_str = "/".join(mid_parts)
                if mid_str: return f"{first}/.../{mid_str}/{last}"
                return f"{first}/.../{last}"
                
            breadcrumb_text = shorten_path(full_path)
            
            lbl_breadcrumb = ctk.CTkLabel(self.explorer_header_frame, text=breadcrumb_text, font=ctk.CTkFont(size=14, weight="bold"), text_color=("#000", "#ffffff"))
            lbl_breadcrumb.pack(side="left", padx=(5, 0))

        # Adicionar o botão Voltar sempre (permitindo navegação livre para cima)
        if self.current_explorer_path and self.current_explorer_path.parent and str(self.current_explorer_path) != str(self.current_explorer_path.parent):
            btn_back = ctk.CTkButton(self.explorer_header_frame, text="⬅️ Voltar", fg_color=("#eeeeee", "#34495e"), hover_color=("#ffffff", "#2c3e50"), text_color=("#000", "#fff"), command=self.explorer_go_back)
            btn_back.pack(fill="x", pady=(0, 10), padx=0)
            
        # Lista as pastas filhas em ordem alfanumérica (natural sort)
        import re
        ...
        try:
            ...
            valid_children = []
            for c in self.current_explorer_path.iterdir():
                if c.name.startswith("."): continue
                if c.is_dir() or (c.is_file() and c.suffix.lower() in image_extensions):
                    valid_children.append(c)
                    
            valid_children.sort(key=natural_sort_key)
            for child in valid_children:
                if child.is_dir():
                    self.create_workspace_item(child)
                else:
                    self.create_workspace_item(child, is_file=True)
        except Exception:
            pass

    def process_thumbnail_queue(self):
        if not hasattr(self, 'thumbnail_queue') or not self.thumbnail_queue:
            self.thumbnail_processing = False
            return
            
        lbl_icon, path = self.thumbnail_queue.pop(0)
        
        if not lbl_icon.winfo_exists():
            # Se o widget já não existe, pula para a próxima sem ler do disco
            self.after(2, self.process_thumbnail_queue)
            return
        ...
        def _bg_load():
            try:
                ...
                if lbl_icon.winfo_exists():
                    lbl_icon.after(0, lambda: lbl_icon.configure(image=ctk_img, text=""))
            except Exception:
                pass
            finally:
                # Chama recursivamente a próxima da fila
                self.after(5, self.process_thumbnail_queue)
    ...
    def hide_image_preview(self):
        ...
        if hasattr(self, 'current_preview_win') and self.current_preview_win is not None:
            if self.current_preview_win.winfo_exists():
                self.current_preview_win.destroy()
            self.current_preview_win = None

    def schedule_image_preview(self, path, x, y):
        self.hide_image_preview()
        # Agenda abertura para 300ms após parar o mouse em cima
        self.preview_timer = self.after(300, lambda: self.show_image_preview(path, x, y))

    def show_image_preview(self, path, x, y):
        ...
        self.current_preview_win = preview_win
        
        preview_win.overrideredirect(True) # Remove as bordas da janela (estilo tooltip)
        preview_win.attributes("-topmost", True) # Mantém no topo absoluto
        preview_win.geometry(f"150x40+{x+20}+{y+10}") # Tamanho miniatura inicial "Carregando"
        
        # Borda sutil preta
        frame_bg = ctk.CTkFrame(preview_win, fg_color=("#f0f0f0", "#111"), corner_radius=0, border_width=1, border_color=("#cccccc", "#555"))
        frame_bg.pack(fill="both", expand=True)
        
        lbl_preview = ctk.CTkLabel(frame_bg, text="Carregando...", font=ctk.CTkFont(size=12))
        lbl_preview.pack(fill="both", expand=True, padx=2, pady=2)
        
        def _load_preview():
            try:
                img = Image.open(path)
                
                # Diminuído levemente para não tampar a tela toda
                max_w, max_h = 500, 700
                ...
                if lbl_preview.winfo_exists():
                    lbl_preview.after(0, lambda: _apply_preview(ctk_img, new_w, new_h))
            except Exception as e:
                pass
                    
        def _apply_preview(ctk_img, w, h):
            if not lbl_preview.winfo_exists(): return
            lbl_preview.configure(image=ctk_img, text="")
            
            screen_w = preview_win.winfo_screenwidth()
            screen_h = preview_win.winfo_screenheight()
            
            # Tenta posicionar à direita do cursor
            pos_x = x + 30
            pos_y = y - (h // 2)
            
            # Se vazar pra direita, joga pra esquerda do cursor
            if pos_x + w > screen_w:
                pos_x = x - w - 30
            
            # Se vazar pra cima/baixo, ajusta aos limites da tela
            if pos_y < 10: pos_y = 10
            if pos_y + h + 40 > screen_h: pos_y = screen_h - h - 40
            
            preview_win.geometry(f"{w+4}x{h+4}+{pos_x}+{pos_y}")
            
        import threading
        threading.Thread(target=_load_preview, daemon=True).start()

    def create_workspace_item(self, path, custom_name=None, custom_icon=None, force_container=False, is_file=False):
        frame = ctk.CTkFrame(self.workspace_list, fg_color=("#e0e0e0" if not is_file else "#f5f5f5", "#333" if not is_file else "#222"), corner_radius=5)
        frame.pack(fill="x", pady=5 if not is_file else 2, padx=5)
        
        if is_file:
            # Travar tamanho (width=36, height=36) evita recálculo de geometria do Tkinter (glitch visual)
            lbl_icon = ctk.CTkLabel(frame, text="🖼️", font=ctk.CTkFont(size=14), width=36, height=36)
            lbl_icon.pack(side="left", padx=(10, 5), pady=4)
            
            # Adiciona miniatura à fila de carregamento (Lazy Load)
            if not hasattr(self, 'thumbnail_queue'):
                self.thumbnail_queue = []
                self.thumbnail_processing = False
                
            self.thumbnail_queue.append((lbl_icon, path))
            
            if not getattr(self, 'thumbnail_processing', False):
                self.thumbnail_processing = True
                self.after(50, self.process_thumbnail_queue)
            ...
            lbl_name = ctk.CTkLabel(frame, text=f"{name} {size_str}", font=ctk.CTkFont(size=12), text_color=("#333", "#aaa"), anchor="w")
            lbl_name.pack(side="left", padx=5, pady=5)
            
            # Binds para hover prévia (Tooltip style)
            lbl_icon.configure(cursor="hand2")
            lbl_name.configure(cursor="hand2")
            lbl_icon.bind("<Enter>", lambda e, p=path: self.schedule_image_preview(p, e.x_root, e.y_root))
            lbl_icon.bind("<Leave>", lambda e: self.hide_image_preview())
            lbl_name.bind("<Enter>", lambda e, p=path: self.schedule_image_preview(p, e.x_root, e.y_root))
            lbl_name.bind("<Leave>", lambda e: self.hide_image_preview())
            
            # Botão > para encaminhar para processamento
            btn_forward = ctk.CTkButton(frame, text=">", width=30, font=ctk.CTkFont(weight="bold"), command=lambda p=path: self.action_process_workspace(p))
            btn_forward.pack(side="right", padx=(10, 10), pady=5)
            
            return # Arquivo final, não renderiza botões do diretório
        ...
        try:
            for child in folder_path.iterdir():
                if child.is_dir() and not child.name.startswith("."):
                    has_subdirs = True
                elif child.is_file() and child.suffix.lower() in [".png", ".jpg", ".jpeg", ".webp", ".bmp"]:
                    has_images = True
        ...
        is_container = force_container or has_subdirs or has_images
        show_processing = has_images
        
        # Lógica de Status (Semáforo) apenas para Pastas com Imagens
        status_color = "#e74c3c"
        if show_processing:
        ...
        if show_processing:
            ...
            chk = ctk.CTkCheckBox(header_frame, text="", variable=var_chk, width=20, checkbox_width=16, checkbox_height=16, bg_color="transparent")
            chk.pack(side="left", padx=(0, 5))
            
            lbl_status = ctk.CTkLabel(header_frame, text="●", text_color=status_color, font=ctk.CTkFont(size=14), bg_color="transparent")
            lbl_status.pack(side="left", padx=(0, 5))
            
        if is_container:
            # Container tem um ícone especial ou padrão
            lbl_icon = ctk.CTkLabel(header_frame, text=custom_icon if custom_icon else "📁", font=ctk.CTkFont(size=14), bg_color="transparent", cursor="hand2")
            lbl_icon.pack(side="left", padx=(0, 5))
            lbl_icon.bind("<Button-1>", lambda e, p=folder_path: self.explorer_enter(p))
        
        display_name = custom_name if custom_name else folder_path.name
        lbl = ctk.CTkLabel(header_frame, text=display_name, font=ctk.CTkFont(weight="bold"), anchor="w", justify="left", bg_color="transparent", cursor="hand2")
        
        btn_frame = ctk.CTkFrame(frame, fg_color="transparent")
        
        # Comportamento principal: Clicar no ícone ou nome SEMPRE entra na pasta
        if is_container:
            lbl.bind("<Button-1>", lambda e, p=folder_path: self.explorer_enter(p))
        ...
        if show_processing:
            ...
            # Botão de sanfona (expansor)
            btn_expand = ctk.CTkButton(header_frame, text="▼", width=24, height=24, fg_color="transparent", hover_color=("#ccc", "#444"), text_color=("#333", "#aaa"), command=toggle_frame)
            btn_expand.pack(side="right", padx=(5, 5))
            
        # Empacotamos o texto por ÚLTIMO, garantindo que a seta preserve seu espaço
        lbl.pack(side="left", fill="x", expand=True, anchor="w")
            
        if show_processing:
            btn_proc = ctk.CTkButton(btn_frame, text="⚙️ Processar", width=40, font=ctk.CTkFont(size=11), command=lambda p=folder_path: self.action_process_workspace(p))
    ...
    def action_process_queue(self):
        ...
        if not selected:
            self.set_tab("Processamento")
            self.log("\n Fila Vazia: Marque pelo menos um capítulo para processar.")
            return
    ...
    def action_editor_workspace(self, folder_path):
        ...
        if target:
            ...
            self.log(f" Nenhum texto OCR encontrado na Temp para esta pasta. Extraia o texto primeiro.")
            self.set_tab("Processamento")
            self.entry_path.delete(0, "end")
            self.entry_path.insert(0, str(folder_path))
    ...
    def action_studio_workspace(self, folder_path):
        ...
        if target:
            ...
            self.log(f" Nenhum texto traduzido encontrado na Temp. Processe a tradução primeiro.")
            self.set_tab("Processamento")
            self.entry_path.delete(0, "end")
            self.entry_path.insert(0, str(folder_path))
    ...
    def setup_process_tab(self):
        ...
        self.cb_pause_ocr = ctk.CTkCheckBox(self.frame_opts, text="Pausar após OCR (Ir para Editor OCR)", variable=self.var_pause_ocr)
        self.cb_pause_ocr.pack(anchor="w", padx=45, pady=(0, 5))

        self.var_correct = ctk.BooleanVar(value=True)
        self.cb_correct = ctk.CTkCheckBox(self.frame_opts, text="2. Polir Texto Extraído (IA Local)", variable=self.var_correct)
        self.cb_correct.pack(anchor="w", padx=20, pady=(10, 5))

        self.var_translate = ctk.BooleanVar(value=False)
        self.cb_translate = ctk.CTkCheckBox(self.frame_opts, text="3. Traduzir para PT-BR (IA Local)", variable=self.var_translate)
        ...
        self.lbl_process_status_cuda = ctk.CTkLabel(self.frame_discreet_status, text="GPU:  Verificando...", font=ctk.CTkFont(size=11), text_color=("#666", "#aaa"))
        self.lbl_process_status_cuda.pack(side="left", padx=10)
        
        self.lbl_process_status_rag = ctk.CTkLabel(self.frame_discreet_status, text="RAG:  Verificando...", font=ctk.CTkFont(size=11), text_color=("#666", "#aaa"))
        ...
        self.btn_reset_process = ctk.CTkButton(self.frame_buttons, text=" Limpar", height=45, width=100, fg_color=("#cccccc", "#555"), hover_color=("#666", "#777"), command=self.reset_processing_tab)
        ...
        # Console (Log)
        self.textbox_log = ctk.CTkTextbox(self.scroll_process, height=320, state="disabled", font=ctk.CTkFont(family="Consolas", size=12))
    ...
    def setup_studio_tab(self):
        ...
        self.frame_studio_toolbar = ctk.CTkFrame(self.frame_studio_main, fg_color=("#e9f7ef", "#183A28"), corner_radius=8)
        self.frame_studio_toolbar.pack(fill="x", pady=(0, 10), padx=5)

        self.lbl_studio_title = ctk.CTkLabel(self.frame_studio_toolbar, text=" ESTÚDIO DE TRADUÇÃO", font=ctk.CTkFont(size=15, weight="bold"), text_color=("#1e8449", "#77CC99"), width=250, anchor="w")
        ...
        # Atalhos do Estúdio
        self.bind("<Control-s>", lambda e: self.save_studio_text() if self.current_tab == "Estúdio de Tradução" else None)
        self.bind("<Left>", lambda e: self.navigate_studio_page(-1) if self.current_tab == "Estúdio de Tradução" else None)
        self.bind("<Right>", lambda e: self.navigate_studio_page(1) if self.current_tab == "Estúdio de Tradução" else None)
        self.bind("<Up>", lambda e: self.navigate_studio_page(-1) if self.current_tab == "Estúdio de Tradução" else None)
        self.bind("<Down>", lambda e: self.navigate_studio_page(1) if self.current_tab == "Estúdio de Tradução" else None)
        ...
        self.textbox_studio_original = ctk.CTkTextbox(self.frame_studio_txt, height=80, font=ctk.CTkFont(size=14), fg_color=("#ffffff", "#333"), text_color=("#555", "#ccc"))
        ...
        # Atalhos de Teclado (QoL)
        def is_typing(event):
            focused = self.focus_get()
            if isinstance(focused, (tk.Entry, tk.Text, ctk.CTkEntry, ctk.CTkTextbox)):
                return True
            try:
                if "textbox" in str(focused).lower() or "entry" in str(focused).lower():
                    return True
            except: pass
            return False

        def on_ctrl_s(event):
            if self.current_tab == "Estúdio de Tradução":
                self.save_studio_text()
                
        def on_right(event):
            if self.current_tab == "Estúdio de Tradução" and self.studio_pages_original:
                if is_typing(event): return
                pages = list(self.studio_pages_original.keys())
                if self.studio_current_page in pages:
                    idx = pages.index(self.studio_current_page)
                    if idx + 1 < len(pages):
                        self.select_studio_page(pages[idx+1])
    ...
    def _load_studio_data(self, folder_path, txt_path=None):
        ...
        if getattr(self, 'studio_is_editor_mode', False):
            self.frame_studio_toolbar.configure(fg_color=("#ebf5fb", "#1B263B"))
            self.lbl_studio_title.configure(text=" EDITOR OCR", text_color=("#2874a6", "#88AADD"))
            self.btn_save_continue.pack(side="right", padx=(10, 10), pady=10)
            ...
            self.frame_studio_toolbar.configure(fg_color=("#e9f7ef", "#183A28"))
            self.lbl_studio_title.configure(text=" ESTÚDIO DE TRADUÇÃO", text_color=("#1e8449", "#77CC99"))
    ...
    def save_studio_text(self):
        ...
        if hasattr(self, 'studio_images_paths') and self.studio_images_paths:
            ...
            try:
                ...
                msg += f"\n Falha ao salvar cópia no Scanlator: {e}"
        ...
        try:
            ...
            self.log(f" Erro ao atualizar Memória RAG: {e}")
    ...
    def start_block_drag(self, block, event):
        ...
        self.drag_placeholder = ctk.CTkFrame(target_frame, height=h, 
                                             fg_color="transparent", border_width=2, border_color=("#cccccc", "#555"), corner_radius=5)
    ...
    def cancel_processing(self):
        if hasattr(self, 'current_process') and self.current_process is not None:
            self.log("\n CANCELAMENTO SOLICITADO. Abortando processo atual...")
    ...
    def start_processing(self):
        ...
        if not (run_ocr or run_correct or run_translate):
            self.log(" Selecione pelo menos uma etapa de processamento nas caixas de seleção.")
            return
        ...
        try:
            ...
            if run_ocr:
                ...
                if p.suffix.lower() == ".txt":
                    self.log(" Você selecionou um arquivo .txt mas deixou o Extrator de Imagem marcado.")
            ...
            if run_correct:
                if not is_ollama_awake():
                    self.log("\n ERRO: O servidor do Ollama não está respondendo!")
            ...
            self.log(f" Tempo Total: {total_seconds:.1f} segundos")
            ...
            self.log("\n==================================================")
            self.log(f" TEMPO TOTAL DA PIPELINE: {elapsed_time:.1f} segundos")
            if balloon_count > 0:
    ...
    def check_module_rag(self):
        try:
            if getattr(sys, 'frozen', False):
                ...
                install_dir = Path(__file__).parent
            site_packages = install_dir / "venv_ui" / "Lib" / "site-packages"
            if not site_packages.exists(): return False
            if not (site_packages / "chromadb").exists(): return False
            if not (site_packages / "sentence_transformers").exists(): return False
            return True
        except:
            return False
    ...
    def check_module_translation(self):
        ...
        # Verifica se o servidor do Ollama está respondendo
        try:
            req = urllib.request.Request("http://127.0.0.1:11434/")
            with urllib.request.urlopen(req, timeout=1) as response:
                if response.status == 200:
                    return "INSTALLED_AND_RUNNING"
        except Exception:
            pass
            
        return "INSTALLED_BUT_CLOSED"
    ...
    def start_ollama_server(self):
        self.btn_install_trans.configure(state="disabled", text="Iniciando...")
        import os
        from pathlib import Path
        import urllib.request
        import time
        ...
        def ping_loop():
            ...
            try:
                ...
                if ollama_app.exists():
                    # Inicia o app oficial (que traz o ícone da bandeja, mas pula na tela)
                    subprocess.Popen([str(ollama_app)], creationflags=0x08000000)
                    
                    # Hack sugerido: O Ollama força a janela. Vamos fechá-la instantaneamente.
                    # Este comando PowerShell fica em loop por 3s verificando a cada 50ms se a janela surgiu.
                    # Assim que ela surge, ele fecha imediatamente.
                    ps_script = (
                        "$timeout = (Get-Date).AddSeconds(3); "
                        "while ((Get-Date) -lt $timeout) { "
                        "Get-Process '*ollama*' -ErrorAction SilentlyContinue | Where-Object {$_.MainWindowHandle -ne 0} | ForEach-Object { $_.CloseMainWindow() }; "
                        "Start-Sleep -Milliseconds 50 }"
                    )
                    
                    def close_window_hack():
                        subprocess.run(["powershell", "-command", ps_script], creationflags=0x08000000)
                        
                    threading.Thread(target=close_window_hack, daemon=True).start()
                else:
                    subprocess.Popen(["ollama", "serve"], creationflags=0x08000000)
            except Exception as e:
                print("Erro ao iniciar Ollama:", e)
                
            # Ping loop (tenta até 15 vezes, 1 segundo de intervalo = 15s de timeout)
            for _ in range(15):
                time.sleep(1)
                try:
                    req = urllib.request.Request("http://127.0.0.1:11434/")
            ...
            # Quando a porta responder (ou der timeout), avisa a interface principal e fecha o console de erro
            self.after(0, self.refresh_module_status)
            self.after(0, lambda: self.log_trans.pack_forget())
    ...
    def open_recommended_models_window(self, refresh_only=False):
        ...
        recommended = ["llama3.1:8b", "phi3:latest", "gemma2:9b", "qwen2.5:14b", "mistral-nemo:12b", "aya:8b"]
        missing_models = [m for m in recommended if m not in installed_models]
        
        # Se for apenas refresh de UI e a janela existir
        if refresh_only and hasattr(self, 'recommended_top') and self.recommended_top.winfo_exists():
            ...
            top.title("Modelos Recomendados")
        ...
        if not missing_models:
            ctk.CTkLabel(top, text="Você já possui todos os modelos recomendados!", text_color="#2ecc71", font=ctk.CTkFont(size=14)).pack(pady=20)
            return
    ...
    def perform_auto_update(self, download_url):
        ...
        center_window(top, 400, 150, self)
        ...
        def worker():
            try:
                ...
                from tkinter import messagebox
                self.after(0, lambda em=err_msg: messagebox.showerror("Erro de Atualização", f"Falha ao baixar/instalar: {em}"))
    ...
    def setup_reader_tab(self):
        ...
        self.lbl_reader_file = ctk.CTkLabel(top_bar, text="Nenhum arquivo selecionado", text_color=("#555", "#888"), font=ctk.CTkFont(weight="bold"))
    ...
    def reader_refresh_list(self):
        ...
        if not biblioteca_dir.exists():
            lbl_empty = ctk.CTkLabel(self.reader_list_frame, text="Biblioteca vazia.", text_color=("#555", "#888"))
        ...
        if not txt_files:
            lbl_empty = ctk.CTkLabel(self.reader_list_frame, text="Nenhum arquivo finalizado.", text_color=("#555", "#888"))
            lbl_empty.pack(pady=20)
            return
        ...
        for fpath in txt_files:
            btn = ctk.CTkButton(self.reader_list_frame, text=" " + fpath.name, anchor="w", fg_color="transparent", hover_color=("#e0e0e0", "#444"), text_color=("#444", "#ddd"), command=lambda p=fpath: self.reader_load_from_sidebar(p))
            btn.pack(fill="x", pady=2)
    ...
    def setup_modules_tab(self):
        ...
        ctk.CTkLabel(header_frame, text="Gerenciador de Módulos (IA)", font=ctk.CTkFont(size=20, weight="bold")).pack(side="left")
        
        self.btn_update_app = ctk.CTkButton(header_frame, text=" Verificar Atualizações", width=160, command=self.check_for_updates)
        self.btn_update_app.pack(side="right", padx=(5, 0))

        self.btn_info_modules = ctk.CTkButton(header_frame, text=" Guia", width=100, command=self.open_modules_guide)
        self.btn_info_modules.pack(side="right", padx=(5, 5))

        self.btn_verify_all = ctk.CTkButton(header_frame, text=" Verificar Módulos", width=150, command=self.refresh_module_status)
        self.btn_verify_all.pack(side="right", padx=(0, 5))
        ...
        try:
            ...
            with open(version_path, "r", encoding="utf-8") as f:
                ...
                current_version_str = ver_data.get('version', 'v1.0.0')
                if not current_version_str.startswith('v'):
                    current_version_str = f"v{current_version_str}"
        except Exception:
            pass


    # Espaço extra no final para evitar corte na fonte itálica do tkinter
        self.lbl_app_version = ctk.CTkLabel(self.frame_modules, text=f"Versão: {current_version_str} ", font=ctk.CTkFont(size=12, slant="italic"), text_color=("#666", "#777"))
        self.lbl_app_version.place(relx=0.97, rely=0.06, anchor="ne")

        ctk.CTkLabel(self.frame_modules, text="Instale apenas o que você precisa. Módulos pesados são opcionais.", font=ctk.CTkFont(size=14), text_color=("#555", "#ccc")).pack(anchor="w", pady=(0, 20))

        # Card Armazenamento
        self.card_storage = ctk.CTkFrame(self.frame_modules, fg_color=("#fcfcfc", "#2b2b2b"), border_width=1, border_color=("#cccccc", "#444"), corner_radius=10)
        ...
        self.btn_clear_hf = ctk.CTkButton(self.card_storage, text=" Apagar Modelos OCR/RAG (Cache)", fg_color="#c0392b", hover_color="#e74c3c", width=220, command=self.clear_hf_cache)
        self.btn_clear_hf.pack(side="right", padx=(5, 15), pady=15)


        # Card OCR
        self.card_ocr = ctk.CTkFrame(self.frame_modules, fg_color=("#fcfcfc", "#2b2b2b"), border_width=1, border_color=("#cccccc", "#444"), corner_radius=10)
        ...
        ctk.CTkLabel(self.card_ocr, text="Necessário para a Etapa 1. Florence-2-Large (GPU/CPU). Tamanho: ~2.5 GB", font=ctk.CTkFont(size=12), text_color=("#666", "#aaa")).pack(anchor="w", padx=15, pady=(0, 10))
        ...
        self.log_ocr = ctk.CTkTextbox(self.card_ocr, height=100, state="disabled", fg_color=("#ffffff", "#000"))

        # Card CUDA
        self.card_cuda = ctk.CTkFrame(self.frame_modules, fg_color=("#fcfcfc", "#2b2b2b"), border_width=1, border_color=("#cccccc", "#444"), corner_radius=10)
        ...
        ctk.CTkLabel(self.card_cuda, text="Exclusivo para placas NVIDIA. Acelera drasticamente a extração OCR. Tamanho: ~2.5 GB", font=ctk.CTkFont(size=12), text_color=("#666", "#aaa")).pack(anchor="w", padx=15, pady=(0, 10))
        ...
        self.switch_gpu = ctk.CTkSwitch(row_cuda, text="Forçar Uso da GPU no OCR", variable=self.var_use_gpu, font=ctk.CTkFont(weight="bold"), fg_color=("#e0e0e0", "#444"), progress_color="#2ecc71")
        ...
        self.btn_install_cuda = ctk.CTkButton(row_cuda, text="Baixar Suporte CUDA", fg_color="#2ecc71", hover_color="#27ae60", text_color="#000", text_color_disabled=("#555", "#999"), command=self.install_cuda_support)
        ...
        self.log_cuda = ctk.CTkTextbox(self.card_cuda, height=80, state="disabled", fg_color=("#ffffff", "#000"))

        # Card Translate
        self.card_trans = ctk.CTkFrame(self.frame_modules, fg_color=("#fcfcfc", "#2b2b2b"), border_width=1, border_color=("#cccccc", "#444"), corner_radius=10)
        ...
        ctk.CTkLabel(self.card_trans, text="Necessário para Etapas 2 e 3 (Ollama Local). Tamanho: Variável", font=ctk.CTkFont(size=12), text_color=("#666", "#aaa")).pack(anchor="w", padx=15, pady=(0, 10))
        ...
        available_models = ["Nenhum modelo local"]
        try:
            import urllib.request
            import json
            req = urllib.request.Request("http://127.0.0.1:11434/api/tags")
        ...
        self.btn_del_corr = ctk.CTkButton(frame_model_select_corr, text=" Apagar Local", fg_color="#c0392b", hover_color="#e74c3c", width=60, command=lambda: self.delete_ollama_model(self.combo_model_corr.get()))
        self.btn_del_corr.pack(side="left", padx=(10, 5))
        ...
        self.btn_del_trans = ctk.CTkButton(frame_model_select_trans, text=" Apagar Local", fg_color="#c0392b", hover_color="#e74c3c", width=60, command=lambda: self.delete_ollama_model(self.combo_model_trans.get()))
        self.btn_del_trans.pack(side="left", padx=(10, 5))
        ...
        self.btn_install_trans = ctk.CTkButton(frame_trans_status, text="Baixar Ollama", fg_color="#e94560", hover_color="#c0392b", command=self.open_ollama_site)
        self.btn_install_trans.pack(side="right", padx=(5, 15), pady=15)
        
        # Botão Inteligente de Recomendados movido para perto do instalar
        self.btn_explore_models = ctk.CTkButton(frame_trans_status, text="🔍 Explorar Recomendados", fg_color="#8e44ad", hover_color="#9b59b6", command=self.open_recommended_models_window)
        self.btn_explore_models.pack(side="right", padx=(5, 5), pady=15)
        ...
        ctk.CTkLabel(self.card_rag, text="Busca semântica no histórico de traduções usando ChromaDB. Tamanho: ~1.5 GB", font=ctk.CTkFont(size=12), text_color=("#666", "#aaa")).pack(anchor="w", padx=15, pady=(0, 10))
        ...
        self.sw_rag = ctk.CTkSwitch(row_rag, text="Ativar Memória de Contexto", variable=self.var_use_rag, font=ctk.CTkFont(weight="bold"), fg_color=("#e0e0e0", "#444"), progress_color="#e94560")
    ...
    def setup_settings_tab(self):
        ...
        # Título
        header = ctk.CTkFrame(scroll, fg_color=("#d9d9d9", "#1f1f1f"), corner_radius=10)
        header.pack(fill="x", padx=20, pady=(20, 10))
        ctk.CTkLabel(header, text="Personalização de Interface", font=ctk.CTkFont(size=20, weight="bold")).pack(pady=15)
        
        # Card de Temas
        card_theme = ctk.CTkFrame(scroll, fg_color=("#fcfcfc", "#2b2b2b"), border_width=1, border_color=("#cccccc", "#444"), corner_radius=10)
        card_theme.pack(fill="x", padx=20, pady=10)
        
        def save_and_apply_settings(*args):
            new_theme = self.var_theme_mode.get()
            new_accent = self.var_theme_accent.get()
            
            # O CustomTkinter aplica o modo de aparência em tempo real
            ctk.set_appearance_mode(new_theme)
            if hasattr(self, 'canvas_studio_img'):
                self.canvas_studio_img.configure(bg="#e0e0e0" if new_theme == "Light" else "#1f1f1f")
            
            # Checa se a cor mudou (pois requer restart no CTk)
            if self.app_settings.get("accent") != new_accent:
                from tkinter import messagebox
                messagebox.showinfo("Reinício Necessário", "A alteração da Cor de Destaque exige que o aplicativo seja reiniciado para atualizar todos os botões.")
            
            self.app_settings["theme"] = new_theme
            self.app_settings["accent"] = new_accent
        ...
        # Aparência (Light/Dark)
        row1 = ctk.CTkFrame(card_theme, fg_color="transparent")
        row1.pack(fill="x", padx=20, pady=15)
        ctk.CTkLabel(row1, text="Modo de Fundo (Aparência):", font=ctk.CTkFont(size=14, weight="bold")).pack(side="left")
        
        self.var_theme_mode = ctk.StringVar(value=self.app_settings.get("theme", "Dark"))
        opt_mode = ctk.CTkOptionMenu(row1, values=["Dark", "Light", "System"], variable=self.var_theme_mode, command=save_and_apply_settings)
        opt_mode.pack(side="right")
        
        # Cor de Destaque
        row2 = ctk.CTkFrame(card_theme, fg_color="transparent")
        row2.pack(fill="x", padx=20, pady=(0, 15))
        ctk.CTkLabel(row2, text="Cor de Destaque (Accent):", font=ctk.CTkFont(size=14, weight="bold")).pack(side="left")
        
        self.var_theme_accent = ctk.StringVar(value=self.app_settings.get("accent", "blue"))
    ...
    def open_modules_guide(self):
        ...
        tab_cuda = tabview.add(" Motor OCR & CUDA")
        tab_ollama = tabview.add(" Modelos Ollama")

        # Aba 1: CUDA e OCR
        ctk.CTkLabel(tab_cuda, text="Instalação e Aceleração GPU", font=ctk.CTkFont(size=18, weight="bold")).pack(pady=10)
        t_cuda = ctk.CTkTextbox(tab_cuda, wrap="word", font=ctk.CTkFont(size=14))
        t_cuda.pack(fill="both", expand=True, padx=10, pady=10)
        t_cuda.insert("0.0", "Recomendações de Instalação:\n\n"
                             " É recomendado que o Motor OCR seja instalado antes do Suporte CUDA. O módulo CUDA atualiza o ambiente do OCR "
                             "para otimização em placas de vídeo. A inversão dessa ordem pode resultar em incompatibilidades.\n\n"
                             "Requisitos para Suporte CUDA:\n\n"
                             " A aceleração via hardware é compatível exclusivamente com placas de vídeo NVIDIA.\n"
                             " Hardware AMD (Radeon) ou Intel deve utilizar a versão CPU (configuração padrão do módulo OCR).\n"
                             " Nota: Modelos recentes, como as placas da linha RTX Série 5000, podem apresentar limitação de suporte "
                             "com a versão atual do framework.")
        ...
        t_ollama.configure(state="disabled")
    ...
    def refresh_module_status(self):
        ...
        if hasattr(self, 'btn_pull_corr'): self.btn_pull_corr.configure(state="normal", text=" Puxar")
        if hasattr(self, 'btn_pull_trans'): self.btn_pull_trans.configure(state="normal", text=" Puxar")
        
        self.btn_verify_all.configure(state="disabled", text=" Verificando...", text_color_disabled="#ffffff")
        ...
        def worker():
            import time
            def update_pb(pb, val):
                self.after(0, lambda: pb.set(val))
            ...
            if has_ocr:
                try:
                    ...
                    torch_version_file = install_dir / "venv_ocr" / "Lib" / "site-packages" / "torch" / "version.py"
                    update_pb(self.pb_cuda, 0.6)
                    
                    if torch_version_file.exists():
                        with open(torch_version_file, "r", encoding="utf-8") as f:
                            has_cuda = "+cu" in f.read()
                    else:
                        has_cuda = False
                except:
                    pass
            ...
            def update_ui():
                self.pb_ocr.set(1.0)
                if has_ocr:
                    self.lbl_status_ocr.configure(text="Status: [ ON ] Instalado", text_color="#2ecc71")
                    self.btn_install_ocr.configure(state="disabled", fg_color=("#e0e0e0", "#333"), text_color_disabled=("#555", "#999"), text="Já Instalado")
                    self.btn_uninstall_ocr.configure(state="normal")
                    self.pb_ocr.configure(progress_color="#2ecc71")
                else:
                    self.lbl_status_ocr.configure(text="Status: [ OFF ] Não Instalado", text_color="#e74c3c")
                ...
                if has_cuda:
                    self.lbl_status_cuda.configure(text="Status: [ ON ] Aceleração Ativa", text_color="#2ecc71")
                    ...
                    self.btn_install_cuda.configure(state="disabled", fg_color=("#e0e0e0", "#333"), text_color_disabled=("#555", "#999"), text="Já Instalado")
                    self.btn_uninstall_cuda.configure(state="normal")
                    if hasattr(self, 'lbl_process_status_cuda'):
                        self.lbl_process_status_cuda.configure(text="GPU: [ ON ]", text_color="#2ecc71")
                else:
                    self.lbl_status_cuda.configure(text="Status: [ OFF ] Aceleração Ausente", text_color="#e74c3c")
                    self.pb_cuda.set(0)

                # --- Recarregar modelos listados ---
                try:
                    import urllib.request
                    import json
                    req = urllib.request.Request("http://127.0.0.1:11434/api/tags")
                    with urllib.request.urlopen(req, timeout=1) as response:
                        ...
                        fetched = [m.get("name", "") for m in data.get("models", []) if m.get("name")]
                        if not fetched:
                            fetched = ["Nenhum modelo local"]
                        if hasattr(self, 'combo_model_corr'):
                            self.combo_model_corr.configure(values=fetched)
                            if self.combo_model_corr.get() not in fetched:
                                self.combo_model_corr.set(fetched[0])
                        if hasattr(self, 'combo_model_trans'):
                    ...
                    if hasattr(self, 'lbl_process_status_cuda'):
                        self.lbl_process_status_cuda.configure(text="GPU: [ OFF ]", text_color="#e74c3c")
                    
                self.pb_rag.set(1.0)
                if has_rag:
                    self.lbl_status_rag.configure(text="Status: [ ON ] Instalado", text_color="#2ecc71")
                    self.btn_install_rag.configure(state="disabled", fg_color=("#e0e0e0", "#333"), text_color_disabled=("#555", "#999"), text="Já Instalado")
                    ...
                    if hasattr(self, 'lbl_process_status_rag'):
                        self.lbl_process_status_rag.configure(text="RAG: [ ON ]", text_color="#2ecc71")
                else:
                    self.lbl_status_rag.configure(text="Status: [ OFF ] Não Instalado", text_color="#e74c3c")
                    ...
                    if hasattr(self, 'lbl_process_status_rag'):
                        self.lbl_process_status_rag.configure(text="RAG: [ OFF ]", text_color="#e74c3c")
                
                self.btn_verify_all.configure(state="normal", text=" Verificar Módulos")
        ...
        def _run_test():
            ...
            try:
                ...
                if "CUDA_OK" in result.stdout:
                    ...
                    self._ui_log(self.log_cuda, " AVISO: GPU NVIDIA (CUDA) não foi detectada pelo PyTorch.")
    ...
    def install_cuda_support(self):
        ...
        self.log_cuda.configure(state="normal")
        self.log_cuda.delete("1.0", "end")
        self.log_cuda.insert("end", " [Etapa 1/3]  Preparando instalação da Aceleração CUDA...\n")
        self.log_cuda.configure(state="disabled")
            ...
            try:
                ...
                for line in proc.stdout:
                    ...
                    if ("download" in l or "fetching" in l) and not has_downloaded:
                        self._ui_log(self.log_cuda, " [Etapa 2/3] Baixando pacotes da NVIDIA (~2.5GB, isso pode demorar)...")
                        has_downloaded = True
                    elif ("installing collected" in l or "running setup" in l) and not has_installed:
                        self._ui_log(self.log_cuda, " [Etapa 3/3] Instalando e configurando os núcleos de processamento...")
                        has_installed = True
                ...
                if proc.returncode == 0:
                    self._ui_log(self.log_cuda, "\n[OK] Aceleração CUDA instalada com sucesso!")
                    self.after(0, self.refresh_module_status)
                else:
                    self._ui_log(self.log_cuda, "\n[ERRO] Ocorreu um erro durante a instalação. Detalhes do erro:")
                    for err_line in full_log[-10:]:
                        self._ui_log(self.log_cuda, f"   {err_line[:120]}")
            except Exception as e:
    ...
    def install_ocr(self):
        ...
        def worker():
            try:
                ...
                self._ui_log(self.log_ocr, " [Etapa 1/4]  Preparando ambiente isolado de OCR...")
                ...
                subprocess.run(
                    [python_exe, "-m", "venv", str(venv_ocr_dir)],
                    capture_output=True,
                ...
                self._ui_log(self.log_ocr, " [Etapa 2/4]  Preparando download do Motor de IA...")
                ...
                for line in proc.stdout:
                    ...
                    l = line.lower()
                    if ("download" in l or "fetching" in l) and not has_downloaded:
                        self._ui_log(self.log_ocr, " [Etapa 3/4]  Baixando pacotes (isso pode demorar)...")
                        has_downloaded = True
                    elif ("installing collected" in l or "running setup" in l) and not has_installed:
                        self._ui_log(self.log_ocr, " [Etapa 4/4]  Instalando e configurando arquivos no disco...")
                        has_installed = True
    ...
    def save_model_prefs(self, _=None):
        ...
        try:
            ...
            self.log(f" Erro ao salvar preferências de modelo: {e}")
